...
<header class="app-header relative z-20 px-[clamp(14px,1.6vw,24px)] py-[clamp(8px,1vw,10px)] flex-shrink-0"
  ...
  <div class="flex items-center gap-[clamp(10px,1vw,20px)] min-h-[40px] flex-nowrap overflow-x-auto">
    ...
    <div class="ml-auto flex items-center gap-[clamp(8px,.8vw,12px)] flex-nowrap min-w-0">
      {#each [
        ...
        <div class="relative flex items-center justify-center gap-[clamp(8px,.8vw,12px)] pl-[clamp(12px,1vw,16px)] pr-[clamp(28px,2vw,32px)] h-[32px] bg-[rgba(10,18,34,0.52)] border border-[rgba(88,118,170,0.28)] rounded-full focus-within:border-[rgba(110,150,220,0.44)] focus-within:bg-[rgba(10,18,34,0.66)] transition-colors min-w-[clamp(150px,16vw,182px)] shadow-[inset_0_1px_0_rgba(255,255,255,0.02),0_0_16px_rgba(37,99,235,0.05)]">
          ...
          <select
            ...
            class="appearance-none font-mono text-[clamp(.62rem,.8vw,.7rem)] leading-none bg-transparent border-none text-slate-100 cursor-pointer outline-none pr-3 w-[clamp(82px,9vw,100px)] text-center [&>option]:bg-[#101827]"
          >
      ...
      {#if isBoard($page.url.pathname)}
        <button
          ...
          class="h-[32px] font-mono text-[clamp(.6rem,.72vw,.64rem)] leading-none px-[clamp(10px,1vw,14px)] bg-[rgba(16,28,50,0.76)] text-slate-100 border border-[rgba(88,118,170,0.30)] rounded-full cursor-pointer hover:border-[rgba(110,150,220,0.44)] hover:text-white transition-colors shadow-[0_0_18px_rgba(37,99,235,0.08)] whitespace-nowrap flex-shrink-0"
        >
          ↺ Reset Layout
        </button>
      {/if}

      <div class="h-[32px] text-[clamp(.58rem,.72vw,.63rem)] leading-none px-[clamp(10px,1vw,16px)] flex items-center bg-[rgba(10,18,34,0.42)] border border-[rgba(88,118,170,0.22)] rounded-full text-slate-400 whitespace-nowrap flex-shrink-0">
        {#if isBoard($page.url.pathname)}
